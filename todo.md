...
- [ ] Req/Resp 去重缓存（`seq -> RESP` 的 TTL/LRU）+ 超时兜底（服务端侧）
- [ ] 会话与门禁：未登录拒绝业务 msgId（>=1000），支持“踢旧连接”策略（可配置）
- [ ] 连接恢复最佳实践落地：小差值 Resume 续传；大差值触发全量 SyncSnapshot

## P1（稳定性/性能）

- [ ] Dispatcher：in-flight 的“按连接/按 shard”更细粒度限流 + 更明确的错误码/错误语义
- [ ] Dispatcher：对 handler 执行耗时的分位数统计与慢请求日志（可配置阈值）
- [ ] Netty：WS pipeline 的参数可配置化增强（backlog/recvBuf/allocator 等）
- [ ] 传输：可插拔 transport（tcp/ws 可切换，ws 默认）
- [ ] Protocol：`ENCRYPT/COMPRESS` 协商与实现（算法选择、顺序、密钥/证书策略）

## P2（分布式与工程化）

- [ ] Jobs：提供至少一个 `LeaseProvider` 参考实现（Redis/etcd/ZK 任选其一，作为可选模块）
- [ ] 分布式任务建议：补充 MQ/Stream 的任务分发最佳实践与示例（仅框架/接口，不写业务）
- [ ] Registry：元信息协议完善（transportCaps/endpoints/instanceId）与灰度/权重字段预留
- [ ] IPC：Unix socket / shared-memory 的实现模块（含背压）

## 文档与示例（不含业务逻辑）

- [ ] Quickstart：提供一个最小 `examples/`（echo + resume/snapshot 演示），不包含任何玩法逻辑
- [ ] 文档：补充“如何接入你自己的 admin/网关/登录服务”的集成说明（只写接口与流程）
